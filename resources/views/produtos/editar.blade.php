...
@section('content')
    ...
    <div class="mt-4">
        ...
        <select id="id_tipo" class="block mt-1 w-full border rounded" name="id_tipo" required>
            @foreach ($tipos as $tipo)
                <option value="{{$tipo['id']}}" {{(old('id_tipo') ?? $produto->id_tipo) == $tipo['id'] ? "selected" : "" }}> {{$tipo['nome']}} </option>
            @endforeach
        </select>
    </div>

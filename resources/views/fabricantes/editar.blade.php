...
@section('variables')
    @php
        $title = "Editando Fabricante '".$fabricante->nome . "'";
        $path = 'fabricantes';
        $id = $fabricante->id;
...
@section('content')
    ...
    <div class="mt-4">
        ...
        <x-text-input id="nome" class="block mt-1 w-full" type="text" name="nome" value="{{old('nome') ?? $fabricante->nome}}" required/>
    </div>
    ...
    <div class="mt-4">
        ...
        <select id="pais" class="block mt-1 w-full border rounded" name="pais" required>
            @foreach ($paises as $pais)
                <option value="{{$pais['nome']}}" {{(old('pais') ?? $fabricante->pais) == $pais['nome'] ? "selected" : "" }}> {{$pais['nome']}} </option>
            @endforeach
        </select>
    </div>
